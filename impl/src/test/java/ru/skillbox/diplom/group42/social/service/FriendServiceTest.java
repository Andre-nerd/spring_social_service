...
import static ru.skillbox.diplom.group42.social.util.MappingTestingDataFactory.createTestFriendFromId;
...
class FriendServiceTest {
    ...
    @Test
    void friendBlockInvokesFriendRepositoryFind() {
        Friend blockFriend = createTestFriendFromId(500L);
        try (MockedStatic<SecurityUtil> utilities = Mockito.mockStatic(SecurityUtil.class)) {
            utilities.when(SecurityUtil::getJwtUserIdFromSecurityContext).thenReturn(TEST_ACCOUNT_ID);
            utilities.when(SecurityUtil::getJwtUserFromSecurityContext).thenReturn(user);
            when(friendRepository.findByIdFromAndIdToAndStatusCode(TEST_ACCOUNT_ID, 500L, StatusCode.FRIEND.toString())).thenReturn(testFriend);
            when(friendRepository.findByIdFromAndIdToAndStatusCode(500L, TEST_ACCOUNT_ID, StatusCode.FRIEND.toString())).thenReturn(blockFriend);
            service.block(500L)
            log.info(testFriend.getId().toString());
            log.info(blockFriend.getId().toString());
        }
    }
}